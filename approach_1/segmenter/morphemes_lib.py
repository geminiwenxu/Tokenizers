    ...
    while len(strategy) > 0:
        ...
        if strategy_leg == "prefix":
            if len(ret_results["word_components_potential"]) > 0:
                ...
                max_entry, max_px, all_rx = find_max_entry(strategy_leg, find_prefixes,
                                                           ret_results["word_components_potential"])
                # if max_entry["key"] != "":
                if len(max_entry) > 0:
            ...
            max_entry, max_sx, all_rx = find_max_entry(strategy_leg, find_suffixes,
                                                       ret_results["word_components_potential"])
            # if max_entry["key"] != "":
            if len(max_entry) > 0:
...
morphemes_filename = "/Users/geminiwenxu/PycharmProjects/Tokenizers/approach_1/segmenter/morphemes.json"
